...
#'   court_df <- ov_detect_court(image_file = image_file)
#'
#'   ## best estimate according to score
#'   idxBS <- which.min(sapply(court_df, function(x) x$score))
...
ov_detect_court <- function(image_file, video_file, t = 60,
    ...
    xr <- magick::image_raster(x, frame = 1, tidy = TRUE) %>% dplyr::filter(.data$x %in% seq.int(1, image_width, by = 1),
                                                                            .data$y %in% seq.int(1, image_height, by = 1))
