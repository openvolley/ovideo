...
#' Automatically detect the volleyball court in the image
#'
#' This function is used to detect the volleyball court in an image, to be used with \code{\link{ov_transform_points}}.
#' The court coordinate system is that used in \code{\link[datavolley]{dv_court}}, \code{\link[datavolley]{ggcourt}}, and related functions.
#' Try \code{plot(c(0, 4), c(0, 7), type = "n", asp = 1); datavolley::dv_court()} or \code{ggplot2::ggplot() + datavolley::ggcourt() + ggplot2::theme_bw()} for a visual depiction.
#'
#' @param image_file string: path to an image file (jpg) containing the court image (not required if \code{video_file} is supplied)
...
#' @examples
#' if (interactive()) {
#'   crt <- ov_detect_court(image_file = system.file("extdata/2019_03_01-KATS-BEDS-court.jpg",
#'                           package = "ovideo"))
#'
#' }
#' @export
ov_detect_court <- function(image_file, video_file, t = 60, type = "corners") {
    assert_that(is.string(type))
    type <- match.arg(tolower(type), c("corners")) ##, "10p"))
    if (missing(image_file) || is.null(image_file)) {
        image_file <- ov_video_frame(video_file, t)
    ...
    im <- magick::image_read(image_file)
    imB <- magick2cimg(magick::image_convert(im,colorspace = "gray" ))
    
    houghT = imager::hough_line(imB, data.frame = TRUE, ntheta = 1000)
    subH  = subset(houghT,score > quantile(score, 0.9999))
    par(mfrow = c(1,1))
    plot(imB)
    with(subH,nfline(theta,rho,col="red"))
